...
# linearSearchTime = [0 for x in range(len(input_sizes))]
...
#     linearTime = timeit.timeit(lambda: linearSearch(shuffleList(randList), index, targetElement), number=1000)
#     linearSearchTime[i] = linearTime
#     print("Iteration for LINEAR SEARCH#", i, "Done For INPUT_SIZE:", index)
#     # print(linearSearchTime, end='\n')

        
# #Calculating average time for each input size for linear search
# avg_linear = [sum(linearSearchTime[i]) / 1000 for i in range(len(input_sizes))]
